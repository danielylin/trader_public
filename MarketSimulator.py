...
if __name__ == "__main__":
    indicators = ["rsi", "bb", "aroon"]
    ticker = "SPY"
    start_date = "2021-06-01"
    end_date = "2022-06-23"
    client = ut.get_ameritrade_client()
    df_prices = ut.get_data(
        tick=ticker, time_interval="day", source="td", start_date=start_date,
        end_date=end_date, client=client).sort_index()

    # df = ut.test_get_data(
    #     "td", ticker, period="6mo").sort_index()

    basket = ba.BasketAlgo(
    ...
    df_trades = basket.df_trades
    # print("Trades")
    # print(df_trades)
    df_signals = basket.df_signals
    # print(df_signals)
    # basket = ba.BasketAlgo(
    #     indicators=indicators,
    #     ticker=ticker,
    ...
    #     df_prices=df
    # )
    # basket.get_indicators()
    # basket.get_positions()
    # df_trades = basket.get_trades()
    # df_signals = basket.df_signals
    # print(basket.df_signals)

    # basket.df_signals.to_csv(f"test_data/{ticker}_signals.csv")

    share_count = int(np.floor(100000/df_prices.sort_index().head(1)["close"].item()))

    sim = MarketSimulator(
        df_trades=df_trades,
        data_source="td",
        ticker=ticker,
        start_port_val=100000,
        shares=share_count
    )
    sim.simulate_trades(stop_loss_level=None, trailing=False)
    # sim.simulate_trades_v2(stop_loss_level=None, trailing=False)
    # sim.df_portval.to_csv
    # (f"test_data/{ticker}_portval.csv")
    print(sim.df_portval["portval"])
    print(generate_portfolio_stats(sim.df_portval["portval"], 252, ticker))
    # plt.plot(sim.df_portval.index, sim.df_portval["portval"])
    # plt.show()
